  ...
  return (
    <div className="w-full mt-8" data-testid="results-panel">
      <Card className="border-border overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 border-b border-border bg-card">
          <div className="text-center" data-testid="stat-widely-available">
            ...
            <p className="text-xs text-muted-foreground font-mono">WIDELY AVAILABLE</p>
          </div>
          <div className="text-center" data-testid="stat-newly-available">
            ...
            <p className="text-xs text-muted-foreground font-mono">NEWLY AVAILABLE</p>
          </div>
          <div className="text-center" data-testid="stat-limited">
            ...
            <p className="text-xs text-muted-foreground font-mono">LIMITED</p>
          </div>
        </div>
              ...
              <AccordionItem key={issue.id} value={issue.id} className="border-border">
                <AccordionTrigger className="hover:no-underline" data-testid={`accordion-trigger-${issue.id}`}>
                  <div className="flex items-center gap-3 text-left flex-1">
                    ...
                    {issue.line && (
                      <span className="text-xs text-muted-foreground font-mono" data-testid={`text-location-${issue.id}`}>
                  ...
                  <div className="pl-10 pr-4 pb-2 space-y-2">
                    ...
                    {issue.recommendation && (
                      <div className="mt-2 p-3 bg-muted/50 rounded-md border border-border">
                        ...
                        <p className="text-xs text-muted-foreground" data-testid={`text-recommendation-${issue.id}`}>
                          {issue.recommendation}
                        </p>
                      </div>
                    )}
